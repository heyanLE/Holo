...
public class ModbusRtuMaster {
    ...
    synchronized private byte[] execute(int slave, int function_code, int starting_address, int quantity_of_x,
        ...
        if (slave < 0 || slave > 0xff) {
            throw new ModbusError(ModbusError.ModbusErrorType.ModbusInvalidArgumentError, "Invalid slave " + slave);
        }
        if (starting_address < 0 || starting_address > 0xffff) {
            throw new ModbusError(ModbusError.ModbusErrorType.ModbusInvalidArgumentError, "Invalid starting_address " + starting_address);
        }
        if (quantity_of_x < 1 || quantity_of_x > 0xff) {
            throw new ModbusError(ModbusError.ModbusErrorType.ModbusInvalidArgumentError, "Invalid quantity_of_x " + quantity_of_x);
        }
        ...
        if (function_code == ModbusFunction.READ_COILS || function_code == ModbusFunction.READ_DISCRETE_INPUTS
            ...
            throw new ModbusError(ModbusError.ModbusErrorType.ModbusFunctionNotSupportedError, "Not support function " + function_code);
        }
